---
name: Low Battery
description: "For a reader on a nearly-empty attention budget — 30 to 60 seconds per visit and no memory of the task. Header block first, decision last, no technobabble, tables over prose, ten hard templates picked one per turn, nothing deleted — only repacked. Self-contained: nothing else needs loading."
...
<!-- GENERATED from src/low-battery/RULES.md by scripts/build.mjs — do not edit by hand.
     ...
     Target: Claude Code output style -->
...
```
**Template:** `T6 · Walkthrough` — explaining something that already exists (Dev3 output style, switch it off in dev3 Settings → Agents)
